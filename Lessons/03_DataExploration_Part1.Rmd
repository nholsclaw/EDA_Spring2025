...
> Answer: It loads the package into memory and gives any warnings
...
# It doesn't save to your environment

# or - error cannot find 'home' function
...
dim(USGS.flow.data) # size of dataframe - rows, columns
length(USGS.flow.data) # columns 

head(USGS.flow.data) # first 5 records of data
head(USGS.flow.data, 10) # first 10 records of data
tail(USGS.flow.data, 5) # last 5 records of data
USGS.flow.data[30000:30005, c(3, 8, 14)] # looking at rows 30,000 - 30,005 and columns 3, 8, and 14

class(USGS.flow.data$datetime) # class of column specified
class(USGS.flow.data$discharge.mean)
class(USGS.flow.data$gage.height.mean)

summary(USGS.flow.data)  #could point to column only with $, summary stats by columns

```

What happened to blank cells in the spreadsheet when they were imported into R?
> Answer: some are still blank, some say NA
...
```{r}
USGS.flow.data$site_no <- as.factor(USGS.flow.data$site_no) # changing data type from number to factor
str(USGS.flow.data)
```
...
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") #shows data from wrong year
```
