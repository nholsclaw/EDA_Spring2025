...
levels(USGS.flow.data$discharge.mean.approval) # shows how many types of variables there are
```
...
```{r, fig.height = 3, fig.width = 4}
# range of discharge.mean, grouped into bins - units cubic ft/s
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))

# can put 'aes' function in the 'geom' function too; width of the bins
ggplot(USGS.flow.data) +
...
# allows you to constrain axes - define extent of x axis
...
```{r, fig.height = 3, fig.width = 4}
# kind of like a 'line' histogram but can have multiple distributions, can't put 'aes' in main ggplot function
ggplot(USGS.flow.data) +
  geom_freqpoly(aes(x = gage.height.mean), bins = 50) +
  geom_freqpoly(aes(x = gage.height.min), bins = 50, color = "lightblue") +
  geom_freqpoly(aes(x = gage.height.max), bins = 50,  lty = 5) +
  scale_x_continuous(limits = c(0, 10)) # limit x axis to 10

# specifying color with data
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
# shows continuous data - gage.height.mean separated by type with 'x'
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

# x and y values continuous 
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1))) # group has to do with bins

# width proportional to number of values 
ggplot(USGS.flow.data) +
...
> Answer: for discrete data - bar plots, for continuous you could use the others

## Ending discussion
What did you learn about the USGS discharge dataset today? What separate insights did the different graph types offer?
> Answer: the box plots and scatter plot showed mean discharge increased with gage height, there wasn't much of a patter in discharge over time


How can multiple options for data exploration inform our understanding of our data?

> Answer: It helps us detect patterns in the data - its better to get a bigger picture than just looking at one or few factors.
...
> Answer: There may have been certain events like storms that caused high discharge but for the most part discharge stays pretty constant. 
