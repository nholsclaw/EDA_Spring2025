...
head(NTL.phys.data) # the first few observations of data
summary(NTL.phys.data) # summary of columns
str(NTL.phys.data) # data structures by column
dim(NTL.phys.data) # dimensions of data

class(NTL.phys.data$sampledate) # data is classified as factor
...
```{r, results = "hide"}
vignette("dplyr") # some info about dplyr
```
...
`>=`
`&` - both should be true
`|` - 'or' one or the other should be true
...
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake") # selecting columns at either Paul or Peter Lake
NTL.phys.data.PeterPaul2 <- filter(NTL.phys.data, lakename != "Central Long Lake" & 
                                     ...
                                     lakename != "Ward Lake" & lakename != "West Long Lake") # exclude lakes you don't want
NTL.phys.data.PeterPaul3 <- filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) # pipe operator option 
...
```{r arranging data}
# allows you to arrange by column ascending/descending
