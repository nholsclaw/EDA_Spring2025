...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    x=0, # specifying where you want the text
    y=0.025,
    label=expression(paste("Mean is ", mu,", ", R^{2},'=0.6')) # what you want the label to say
    )
...
PeterPaul.chem.nutrients %>% 
  ...
  geom_bar(aes(x = factor(month, 
                          levels = 1:12, 
                          labels = month.abb)))+
  scale_x_discrete(drop = FALSE, name = "Month")
...
PeterPaul.chem.nutrients %>% 
  ...
  geom_point()+
  facet_wrap(vars(lakename), nrow = 2, ncol = 1)+
  ylab("Temperature \u00b0C")
...
EPAair %>% 
  ggplot(aes(x = PM2.5, y = as.factor(Year)))+
  #geom_boxplot()+
  geom_violin()+
  #geom_dotplot()+
