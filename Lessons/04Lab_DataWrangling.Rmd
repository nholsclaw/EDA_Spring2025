...
NTL.phys.data.90to99 <- filter(NTL.phys.data, year4 %in% c(1990:1999)&
                               lakename == "Tuesday Lake")

```
Question: Why don't we filter using row numbers?

> Answer: too much data to go through 
...
Ex5 <- NTL.phys.data %>% 
  pivot_longer(
    cols = c(irradianceWater:irradianceDeck), # columns you want to combine
    names_to = "irradiance_type" , # new column where names of old columns will go
    values_to = "irradiance" # new column with irrandiance value
  )
